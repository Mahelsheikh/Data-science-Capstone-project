...
Took three samples of 0.5% of the cleaned text and one more sample for testing. These are to be used to create 3 matrices of prewords and current word keeping only unique prewords from the second matrix comparing to the prewords in the first matrix and so on for the thrid matrix as well.  
...
These are 2 wrap up functions for the above functions to calculate katz's probability with and without backoff. The first function is for bigrams and trigrams and the second for quadgrams.  
